...
module.exports = function(sequelize, types) {
	const Asset = sequelize.define("asset", {
		...
		type: types.ENUM('photo', 'raw', 'video'),
		format: types.STRING(10),
		width: types.INTEGER,
	...
	Asset.prototype.canonicalPath = function () {
		const time = this.bornAt || this.createdAt;

		if (time) {
			const m = momentjs(time);
			const year = m.format("YYYY");
			const month = m.format("MM");
			const time = m.format("YYYY.MM.DD.HH.mm.ss.SSSS");
			const uuid = this.uuid.substr(0, 6);
			const file = time + '.' + uuid + '.' + this.format;
