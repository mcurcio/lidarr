		...
		return (async () => {
			if (options.fields.includes('momentId')) {
				await Promise.all([
					...
					(async () => {
						m = await Moment.findById(photo.momentId);
...
//console.log('got ', moments.length, ' moments');
			switch (moments.length) {
				...
				case 1:
//					console.log('match 1 moment', photo.get({plain:true}), moments[0].get({plain:true}));
					await photo.setMoment(moments[0], { save: false });
					break;

				default:
					let moment = moments.shift();
					let photos = await moment.getPhotos();
					let otherPhotos = await Promise.all(moments.map(m => m.getPhotos()));
					photos.concat(otherPhotos);
					await moment.setPhotos(photos);
					await Promise.all(moments.map(m => m.destroy()));
					photo.setMoment(moment, { save: false });
			}
		})();
	});

	Photo.hook('afterCreate', (photo, options) => {
		return (async () => {
			let moment = await photo.getMoment();
			await moment.notifyPhotoAdded(photo, options);
		})();
	});
/*
