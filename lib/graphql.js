...
module.exports = (env) => {
	...
	const computeTotalPages = (args, total) => {
		//console.log('args', args);
		//console.log('fullCount', total);
		const perPage = args.first || args.last || total;
		//console.log('perPage', perPage);
		const pages = perPage ? Math.ceil(total / perPage) : null;
		...
		connectionFields: {
			...
			totalPages: {
				...
				resolve: async ({args}) => {
					...
					return computeTotalPages(args, total);
				}
			},
			currentPage: {
				type: graphql.Int,
				resolve: async ({args, where, edges, fullCount}) => {
					const perPage = args.first || args.last || edges.length;
					//console.log('perPage', perPage);
					const edge = edges[0];
					...
					const total = await db.Moment.count();
					const pages = computeTotalPages(args, total);
					if (!pages) {
						return null;
					}
					const pagesBefore = Math.ceil(count / perPage);
					//console.log('pagesBefore', pagesBefore);
					const page = Math.ceil(pagesBefore / pages);
					//console.log('currentPage', count, pages, page);
					return page;
				}
			}
		}
