...
function syncFile(searchPath, assets, env, options={}) {
	...
	return (async () => {
		for (let a of assets) {
			...
			const tags = await exiftool.read(assetPath);
			const {Orientation} = await exiftool.read(assetPath, ["-Orientation#"]);
			if (!tags) {
			...
			let date = tags.GPSDateTime || tags.DateTimeOriginal ||
				tags.CreationDate ||
				tags.CreateDate || tags.MediaCreateDate ||
				tags.FileInodeChangeDate;
			...
			let [asset, created] = await Asset.findOrCreate({
				...
				defaults: {
					...
					exif: tags,
					bornAt: momentjs(date.toString())
				},
				env
			});
			env.logger.debug({created, asset}, "Created asset");
