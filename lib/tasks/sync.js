...
module.exports = class SyncTask extends BaseTask {
	constructor(directory, db, options) {
		super();
		...
		if (this._options.move && path.relative(this._options.move, this._directory) === "..") {
			reject(new Error("Import directory may not be within library directory"));
		}
	}

	_start() {
		const queue = async.queue((file, cb) => {
			syncFile(this._directory, file, this._db, this._options)
				.then((val) => {
					cb(null, val);
				})
				.catch((err) => {
