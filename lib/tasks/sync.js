...
function syncFile(searchPath, assets, env, options={}) {
	...
	return (async () => {
		for (let a of assets) {
			...
			const [tags, {Orientation}] = await Promise.all([
			...
			if (!date) {
				...
				date = momentjs(path.parse(assetPath).name, Asset.FILE_DATE_FORMATS, true);
				if (!date.isValid()) {
					date = null;
					env.logger.warn({assetPath}, "No date could be found in the filename");

					date = tags.FileInodeChangeDate;
					if (!date) {
						env.logger.error({assetPath}, "No file creation date was available");
						continue;
					} else {
			...
			const values = {
				...
				exif: tags,
				bornAt: momentjs(date.toISOString())
			...
			if (created) {
				env.logger.debug({created, asset, date, moment: momentjs(date.toISOString())}, "Created asset");
			} else {
				await asset.update(values);
				env.logger.debug({asset, values}, "Updated asset");
			}
