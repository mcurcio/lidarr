	...
	_start() {
		const queue = async.queue((asset, cb) => {
			...
			(async () => {
				...
				if (asset.type === 'video') {
					// FIXME: generate real thumbnails
					this._env.logger.warn("Video thumbnails are not yet supported");
					...
					this._env.logger.warn({type:asset.type}, "Unsupported asset type");
				}
			})()
